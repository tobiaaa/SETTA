...
class TSCB(nn.Module):
    def __init__(self, num_channel=64, **kwargs):
...
class TSCNet(nn.Module):
    def __init__(self, num_channel=64, num_features=201, depth=4):
        super(TSCNet, self).__init__()
        self.dense_encoder = DenseEncoder(in_channel=3, channels=num_channel)
        
        self.TSCB = nn.Sequential()
        for i in range(depth):
            self.TSCB.add_module(f'TSCB_{i+1}', TSCB(num_channel=num_channel))
    ...
    def forward(self, x, return_mask=False, return_parts=False):
        ...
        if return_parts:
            return mask, mag, noisy_phase, complex_out
        ...
        if return_mask:
            return (final_real, final_imag), mask
