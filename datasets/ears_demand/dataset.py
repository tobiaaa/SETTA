...
class EARS_DEMAND(Dataset):
    def __init__(self, cfg, data_cfg=None, split='train', transforms=None):
        ...
        self.noise_dir = os.path.join(data_cfg.noise_path, DIR_DICT[self.fs])
    ...
    def __getitem__(self, index):
        ...
        noise_file = os.path.join(self.noise_dir, noise['dir'], noise['file'])
        ...
        assert x_clean.shape[0] == x_noise.shape[0], f'{x_clean.shape}; {x_noise.shape}'
        ...
        if not self.eval_mode:
            ...
            if x_clean.shape[0] <= self.sample_len:
                pad_val = self.sample_len - x_clean.shape[0] + 2
                x_clean = F.pad(x_clean, (pad_val // 2, pad_val // 2, 0, 0))
                x_noisy = F.pad(x_noisy, (pad_val // 2, pad_val // 2, 0, 0))
            # Select random segment
            start = torch.randint(0, x_clean.shape[0] - self.sample_len, ())
