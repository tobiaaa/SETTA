"""
Pieced together from various repos
"""
import numpy as np
import torch
...
class Composite(Metric):
    def calc_comp(self, x_clean, x_denoised):

        wss_dist = wss(x_clean, x_denoised, self.fs)
        llr_mean = llr(x_clean, x_denoised, self.fs)
        segSNR = SNRseg(x_clean, x_denoised, self.fs)
        pesq_mos = pesq(x_clean, x_denoised, self.fs)
    ...
    def update(self, x_clean, x_noisy, x_denoised, kwargs):
        ...
        for x_cl, x_de in zip(x_clean, x_denoised):
            c_sig, c_bak, c_ovl = self.calc_comp(x_cl, x_de)
            update_vals.append([c_sig, c_bak, c_ovl])
        
        update_vals = torch.tensor(update_vals).squeeze(0)
        self.append(update_vals)

        return update_vals 

    def names(self):
        return ['C_sig', 'C_bak', 'C_ovl']
...
def fwSNRseg(cleanSig, enhancedSig, fs, frameLen=0.03, overlap=0.75):
    ...
    hannWin = 0.5*(1-np.cos(2*np.pi*np.arange(1, winlength+1)/(winlength+1)))
    f, t, Zxx = stft(cleanSig[0:int(num_frames)*skiprate+int(winlength-skiprate)], fs=fs, window=hannWin, nperseg=winlength,
    ...
    f, t, Zxx = stft(enhancedSig[0:int(num_frames)*skiprate+int(winlength-skiprate)], fs=fs, window=hannWin, nperseg=winlength,
                     noverlap=winlength-skiprate, nfft=n_fft, detrend=False, return_onesided=True, boundary=None, padded=False)
    enh_spec = np.abs(Zxx)
    enh_spec = enh_spec[:-1, :]
...
def pesq(clean_speech, processed_speech, fs):
    if fs == 8000:
        ...
        # remap to raw pesq score
        pesq_mos = 46607/14945 - \
            (2000*np.log(1/(pesq_mos/4 - 999/4000) - 1))/2989

    elif fs == 16000:
        pesq_mos = pesq_inner(fs, clean_speech, processed_speech, 'wb')
    elif fs >= 16000:
        numSamples = round(len(clean_speech)/fs*16000)
        pesq_mos = pesq_inner(16_000, resample(clean_speech, numSamples),
